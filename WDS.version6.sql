...
CREATE TABLE IF NOT EXISTS `WDS`.`user` (
  `user_id` BIGINT NOT NULL AUTO_INCREMENT COMMENT 'User identification number',
...
CREATE TABLE IF NOT EXISTS `WDS`.`customer` (
  `cid` BIGINT NOT NULL AUTO_INCREMENT COMMENT 'Customer ID',
  `cfirstname` VARCHAR(30) NOT NULL COMMENT 'Customer’s first name',
  `clastname` VARCHAR(30) NOT NULL COMMENT 'Customer’s last name',
  `cgender` CHAR(1) NULL DEFAULT NULL COMMENT 'Customer’s gender, “M”, or “F” representing “Male” or “Female”',
  `cmaritality` CHAR(1) NOT NULL COMMENT 'Customer’s martial status, “M”, “S”, or “W”, representing “Married”, “Single”, and “Widow/Widower” respectively',
  `cinstype` CHAR(1) NOT NULL COMMENT 'Insurance types customer choose, one of “A”, “H”, or “B” representing “Auto”, “Home”, and “Both”.',
  `chouse` INT NOT NULL COMMENT 'House/ Department number',
  `cstreet` VARCHAR(30) NOT NULL COMMENT 'Street address of the house',
  `ccity` VARCHAR(30) NOT NULL COMMENT 'City address of the house',
  `cstate` VARCHAR(2) NOT NULL COMMENT 'State of the house located',
  `czipcode` INT NOT NULL COMMENT 'Zipcode of house address',
  `user_id` BIGINT NOT NULL COMMENT 'User ID of the customer',
  PRIMARY KEY (`cid`),
...
CREATE TABLE IF NOT EXISTS `WDS`.`auto_ins` (
  `insid` BIGINT NOT NULL COMMENT 'Insurance ID',
  `vin` VARCHAR(20) NOT NULL COMMENT 'Vehicle identification number',
  `vmake` VARCHAR(30) NOT NULL COMMENT 'Vehicle Make',
  `vmodel` VARCHAR(30) NOT NULL COMMENT 'Vehicle Model',
  `vyear` INT NOT NULL COMMENT 'Produce Year of vehicle',
  `vstatus` CHAR(1) NOT NULL COMMENT 'Status of the vehicle, one of “L”, “F”, or “O” representing “Leased”, Financed”, “and Owned’.',
  `licenseno` VARCHAR(10) NOT NULL COMMENT 'License number of the driver',
  `driverfirstname` VARCHAR(30) NOT NULL COMMENT 'Driver’s first name',
  `driverlastname` VARCHAR(30) NOT NULL COMMENT 'Driver’s last name',
  `driverbirthdate` DATETIME NOT NULL COMMENT 'Driver’s birth date',
  PRIMARY KEY (`insid`),
...
CREATE TABLE IF NOT EXISTS `WDS`.`home_ins` (
  `insid` BIGINT NOT NULL COMMENT 'Insurance ID',
  `hpurchasedate` DATETIME NOT NULL COMMENT 'House purchase date',
  `hvalue` DECIMAL(10,2) NOT NULL COMMENT 'House value, in dollars',
  `harea` DECIMAL(8,2) NOT NULL COMMENT 'House area, in sqrt feet',
  `htype` CHAR(1) NOT NULL COMMENT 'House type, one of the value as, S,M,C,T representing Single family, Multi Family, Condominium, Town house respectively',
  `hfire` CHAR(1) NOT NULL COMMENT 'There is automatic fire notification or not, “Y” or “N”',
  `hsecurity` CHAR(1) NOT NULL COMMENT 'The home security system is installed and monitored, or not, “Y” or “N”',
  `hpool` CHAR(1) NULL DEFAULT NULL COMMENT 'The swimming pool of the house: “U”, “O”, “I”, “M” or Null, for Underground swimming pool, Overground swimming pool, Indoor swimming pool, Multiple swimming pool, No swimming pool',
  `hbasement` CHAR(1) NOT NULL COMMENT 'There is a basement or not, “Y” or “N”',
  PRIMARY KEY (`insid`),
...
CREATE TABLE IF NOT EXISTS `WDS`.`payment` (
  `payno` BIGINT NOT NULL AUTO_INCREMENT COMMENT 'Payment number',
  `paydate` DATETIME NOT NULL COMMENT 'Payment generated date',
  `paymethod` VARCHAR(10) NOT NULL COMMENT 'Payment method, choose one from “PayPal”, “Credit”, “Debit”, “Check”',
  `payamount` BIGINT NOT NULL COMMENT 'Payment amount',
  `invno` BIGINT NOT NULL COMMENT 'Invoice number for this payment',
  PRIMARY KEY (`payno`),
...
-- -----------------------------------------------------
-- procedure sp_editInsurance
-- -----------------------------------------------------

USE `WDS`;
DROP procedure IF EXISTS `WDS`.`sp_editInsurance`;

DELIMITER $$
USE `WDS`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_editInsurance`(in p_insid bigint, in p_insstartdate varchar(20), 
										in p_insenddate varchar(20), in p_inspremium decimal(10,2))
BEGIN
	update insurance set insstartdate = p_insstartdate, insenddate = p_insenddate, inspremium = p_inspremium where insid = p_insid;
...
USE `WDS`$$
DROP TRIGGER IF EXISTS `WDS`.`insurance_AFTER_UPDATE` $$
USE `WDS`$$
CREATE
DEFINER=`root`@`localhost`
TRIGGER `WDS`.`insurance_AFTER_UPDATE`
AFTER UPDATE ON `WDS`.`insurance`
FOR EACH ROW
BEGIN
	set @temp_amount := (ceiling(datediff(new.insenddate, new.insstartdate)/365)) * new.inspremium;
	update invoice set invdate = now(), invdue = new.insenddate, 
					outstanding = outstanding + @temp_amount - invamount, invamount = @temp_amount
                    where invoice.insid = new.insid;
END$$
